...
The prediction results for the four models are summarized in Table 4 below. The test data is imbalanced with the 0 class having a higher proportion, so the Cohen’s Kappa instead of accuracy us focused to evaluate model performance. It’s shown that all neural network models, along with the baseline random forest model, have similar Cohen’s Kappa values. RNN model outperforms the other models slightly with a Cohen’s Kappa of 0.53. The Cohen’s Kappa values indicate that our models have predictive power, rather than assigning labels randomly. In addition, improvements were also seen for other metrics. 
